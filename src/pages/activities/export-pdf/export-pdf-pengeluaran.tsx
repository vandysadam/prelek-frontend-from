...
const styles = StyleSheet.create({
  ...
  celluraian: { margin: 5, fontSize: 11, flex: 2 },
...
// Komponen PDF
const ReportPDF: React.FC<ReportPDFProps> = ({ transaksiByBulan, tahun, totalTahun }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      {/* Judul Laporan */}
      <Text style={styles.title}>Laporan Pengeluaran Tahun {tahun}</Text>
          ...
          <View key={bulan} style={styles.section}>
            ...
            {transaksiBulan.map((item, index) => (
              <View key={index} style={styles.row}>
                ...
                <Text style={styles.cell}>Rp {item.total_pengeluaran.toLocaleString()}</Text>
              </View>
            ))}

            {/* Total Pengeluaran Per Bulan */}
            <Text style={styles.total}>Total Pengeluaran {bulan}: Rp {totalPerBulan.toLocaleString()}</Text>
          </View>
        );
      })}
            <View style={styles.row}/>
      ...
      <Text style={styles.total}>Total Pengeluaran Tahun {tahun}: Rp {totalTahun.toLocaleString()}</Text>
    </Page>
  </Document>
);
