





import DashboardLayout from "@/layouts/dasboard-layout";
import { useGetTotalQuery } from "../../../modules/users/api/user.api";
import { useParams } from "react-router-dom";
import { PDFDownloadLink } from "@react-pdf/renderer";
import ReportPDFTotal from "./export-pdf/export-pdf-total";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader } from "@/components/ui/card";

type Transaksi = {
    tanggal: string;
    uraian: string | null;
    jenis_transaksi: string;
    total_pemasukan: number;
    total_pengeluaran: number;
    bulan: string;
}
    const ReportPageTotal : React.FC = () => {
        const { year } = useParams<{ year: string }>();
        console.log('ini isi dari id ', year);
        const tahun : string = year!;

        const { data, error, isLoading } = useGetTotalQuery(tahun);
        console.log('data dari api ', data)
        console.log("Year parameter: ", tahun);

    if(isLoading){
        return (
            <DashboardLayout>
                <h1>Loading...</h1>
            </DashboardLayout>
        );
    }
    if (error) {
        return (
          <DashboardLayout>
            <h1>Error fetching data</h1>
          </DashboardLayout>
        );
      }
      // const transaksi = Array.isArray(data?.data) ? data.data : [];
      const transaksi: Transaksi[] = Array.isArray(data?.data) ? data.data : [];

      const transaksiByBulan = transaksi.reduce((acc: Record<string, Transaksi[]>, item: Transaksi) => {
        // const kelompok = item.bulan;
        const bulan = new Date(item.bulan).toLocaleString("id-ID", { month: "long", year: "numeric" });
        if (!acc[bulan]) {
          acc[bulan] = [];
        }
        acc[bulan].push(item);
        return acc;
      }, {});
    
      const bulanOrder = [
        "Januari", "Februari", "Maret", "April", "Mei", "Juni", 
        "Juli", "Agustus", "September", "Oktober", "November", "Desember"
    ];


    const totalTahunPemasukan = Object.values(transaksiByBulan).reduce((total, transaksiBulanIni) => {
      return total + transaksiBulanIni.reduce((bulanTotal, item) => {
        const pemasukan = parseFloat(item.total_pemasukan.toString());
        return bulanTotal + (isNaN(pemasukan) ? 0 : pemasukan);
      }, 0);
    }, 0);

    const totalTahunPengeluaran = Object.values(transaksiByBulan).reduce((total, transaksiBulanIni) => {
        return total + transaksiBulanIni.reduce((bulanTotal, item) => {
          const pemasukan = parseFloat(item.total_pengeluaran.toString());
          return bulanTotal + (isNaN(pemasukan) ? 0 : pemasukan);
        }, 0);
      }, 0);

      
        
 
      
    return (
        <DashboardLayout>
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
              <h1 className="text-2xl font-bold">Report Page Ringkasan Tahun {tahun}</h1>
                  <PDFDownloadLink
                      document={<ReportPDFTotal transaksiByBulan={transaksiByBulan}  tahun={tahun} totalPemasukan={totalTahunPemasukan}  totalPengeluaran={totalTahunPengeluaran}/>}
                      fileName={`Laporan_Pengeluaran_${tahun}.pdf`}
                      className="btn btn-primary">
                      <Button className="px-4 py-2  text-white rounded ">
                          Download Laporan
                      </Button>
                  </PDFDownloadLink>
              </div>
            </CardHeader>
            <br/>       
            <CardContent>     
            <div>
                <table className="table-auto w-full border-collapse border">
                    <thead>
                    <tr>
                        <th className="border px-4 py-2">No</th>
                        <th className="border px-4 py-2">Bulan</th>
                        <th className="border px-4 py-2">Pemasukan</th>
                        <th className="border px-4 py-2">Pengeluaran</th>
                    </tr>
                    </thead>
                    <tbody>
                    {bulanOrder.map((kelompok, index) => {
                        const bulanFormatted = `${kelompok} ${tahun}`;
                        const transaksiBulanIni = transaksiByBulan[bulanFormatted] || [];

                        // Hitung total pemasukan dan pengeluaran per bulan
                        const totalBulan = transaksiBulanIni.reduce((total, item) => {
                        const pemasukan = parseFloat(item.total_pemasukan.toString());
                        return total + (isNaN(pemasukan) ? 0 : pemasukan);
                        }, 0);

                        const totalBulanPengeluaran = transaksiBulanIni.reduce((total, item) => {
                        const pengeluaran = parseFloat(item.total_pengeluaran.toString());
                        return total + (isNaN(pengeluaran) ? 0 : pengeluaran);
                        }, 0);

                        // Tampilkan baris data untuk bulan ini
                        return (
                        <tr key={kelompok}>
                            <td className="border px-4 py-2">{index + 1}</td>
                            <td className="border px-4 py-2">{kelompok}</td>
                            <td className="border px-4 py-2">Rp {totalBulan.toLocaleString()}</td>
                            <td className="border px-4 py-2">Rp {totalBulanPengeluaran.toLocaleString()}</td>
                        </tr>
                        );
                    })}
                    </tbody>
                    <tfoot className="text-bold">
                    <tr>
                        <td className="border px-4 py-2" style={{ fontWeight: "bold", fontSize: "20px"  }}>Total</td>
                        <td className="border px-4 py-2"></td>
                        <td className="border px-4 py-2" style={{  fontWeight: "bold", fontSize: "20px"  }}>Rp {totalTahunPemasukan.toLocaleString()}</td>
                        <td className="border px-4 py-2" style={{ fontWeight: "bold", fontSize: "20px"  }}>Rp {totalTahunPengeluaran.toLocaleString()}</td>
                    </tr>
                    </tfoot>
                </table>
                </div>
                {/* <div className="mt-4 px-40 text-right text-xl">
                    <span>Total Pengeluaran Tahun {tahun} : Rp {totalTahunPemasukan.toLocaleString()}</span><br/>
                    <span>Total Pengeluaran Tahun {tahun} : Rp {totalTahunPengeluaran.toLocaleString()}</span>
                </div> */}
              </CardContent>
          </Card>
        </DashboardLayout>
    );
  };

export default ReportPageTotal;