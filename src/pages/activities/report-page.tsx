...
import { useGetPemasukanQuery } from "../../../modules/users/api/user.api";
import { useParams } from "react-router-dom";
import ReportPDFPemasukan from "./export-pdf/export-pdf-pemasukan";
...
type Transaksi = {
    ...
    const ReportPage : React.FC = () => {
        ...
        const { data, error, isLoading } = useGetPemasukanQuery(tahun);
    ...
    return (
        <DashboardLayout>
          ...
          <Card>
            <CardHeader>
              ...
              <h1 className="text-2xl font-bold">Report Page Pemasukan Tahun {tahun}</h1>
              <PDFDownloadLink
                      document={<ReportPDFPemasukan transaksiByBulan={transaksiByBulan} tahun={tahun} totalTahun={totalTahun} />}
                    ...
                    <tbody>
                      {transaksiBulanIni.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="border px-4 py-2">Rp {item.total_pemasukan}</td>
                        </tr>                      
                      ))} 
                    </tbody>                  
                    </table>
                  <div className="mt-4 px-40 text-right text-xl">
                    <span>Total pemasukan : Rp {totalBulan}</span>
                  </div>
                </>
              )}
            </div>
      ...
      <div className="mt-4 px-40 text-right text-xl">
        <span>Total Pemasukan Tahun {tahun} : Rp {totalTahun.toLocaleString()}</span>
      </div>
      </CardContent>
      </Card>
      </div>
